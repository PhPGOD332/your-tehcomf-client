...
const workStages: WorkStage[] = [
	{
		title: 'Концепция',
		description: [
			'Изучаем дизайн-проект заказчика;',
			'Определяем назначение мебели, габариты, стиль, цветовую гамму и бюджет;',
			'Формируем внешний вид и функционал будущего изделия с учетом технических параметров проекта, пожеланий и необходимых материалов;',
			'При необходимости делаем перезамер помещения, чтобы исключить ошибки при проектировании;',
	...
	},
	{
		...
		description: [
			...
			'Деталировка. Разработка чертежей каждого элемента мебели.',
			'Создание спецификаций: составление полного списка деталей, необходимого количества плитного материала, фурнитуры, крепежных элементов и аксессуаров.',
	...
	},
	{
		...
		description: [
			'Распил плитных материалов на детали нужного размера согласно картам раскроя (на форматно-раскроечных станках или станках с ЧПУ).',
			'Кромление (облицовка торцов): нанесение кромки на открытые срезы деталей для защиты от влаги и придания эстетичного вида, которую мы клеим исключительно на PUR.',
			'Присадки (сверление): выполнение технологических отверстий под крепеж (петли, стяжки, шканты) и фурнитуру.',
