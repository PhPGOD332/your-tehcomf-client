...
const PopupForm = ({
	...
	const bgPopupHandler = (e: React.MouseEvent<HTMLDivElement>) => {
		if (e.target === popupBgRef.current) setIsOpen(false);
	};
	...
	const touchEndHandle = () => {
		...
		if (popupDragOffset > popupFormRef.current.offsetHeight / 2) {
			setIsOpen(false);
		} else {
	...
	const claimTypeChooseBlock = (
		<div className={styles.chooseWrapper}>
			...
			{errors.claimType && (
				<span className={styles.claimTypeError}>
		...
		<>
			{!isMobile ? (
				...
				>
					<form
						className={
							isOpen ? styles.popupContent : styles.popupWrapper_hidden
						}
						ref={popupFormRef}
					>
						...
						<div className={styles.submitBlock}>
							<span className={styles.politicsSpan}>
								Я согласен(на) на{' '}
								<Link
